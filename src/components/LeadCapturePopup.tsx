...
export default function LeadCapturePopup() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-navy/50 px-3 py-4">
      <div className="w-full max-w-md bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
        <div className="flex items-start justify-between gap-4 px-5 py-4 border-b border-gray-100">
          <div>
            <p className="text-xs font-bold text-teal uppercase tracking-wider">Taxaltus Companion</p>
            <h2 className="text-lg font-bold text-navy mt-1">Get filing reminders and tax updates</h2>
          </div>
          ...
          <button
            ...
            onClick={close}
            aria-label="Close"
            className="shrink-0 p-2 rounded-full hover:bg-gray-100 text-gray-500"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <form onSubmit={submit} className="px-5 py-5 space-y-4">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full bg-teal hover:bg-teal-700 disabled:opacity-60 text-white font-semibold rounded-xl px-4 py-3 transition-colors"
          >
